...
class Textarea extends \control\fields\Textarea
{
    ...
    public static function generateScript(){
        ...
        <script type="text/javascript">
            (function($,editor){
                editor.generate_textarea_field = function(fieldKey,data,cel){
                    data = $.extend({
                        'title':'未命名',
                        'description':'',
                        'required':false,
                    ...
                    return '<div data-field="'+fieldKey+'" data-type="textarea" class="form-editor-field fui-form-builder--col fui-form-builder--col-'+(12/cel)+' ui-draggable ui-draggable-handle" >' +
                                '<div class="fui-form-field forminator-field-type_name">'+
                                    ...
                                    '<textarea rows="3" class="sui-form-control" style="'+data.style+'" placeholder="'+data.placeholder+'"></textarea>'+
                                    '<span class="sui-description">'+data.description+'</span>'+
                                '</div>'+
                            '</div>';
                };

                editor.get_textarea_config=function(fieldKey,data){
                    return {
                        'base':{
                            'title':'标准',
                            'options': {
