...
class Decimal extends \control\fields\Decimal
{
    /**
     * 对数据查询进行过滤
     *
    ...
    public function where(&$joins,&$where,$primaryTable,$primaryIdName,$objectType,$value){
        ...
        if($this->get('searchable')){
            $joins["module_form_decimal form_{$this->key}"] = "form_{$this->key}.object_type='{$objectType}' and form_{$this->key}.object_id={$primaryTable}.{$primaryIdName} and form_{$this->key}.field_key='{$this->key}'";
            $where[]=[
    ...
    public function sort(&$joins,&$where,&$sorts,&$groupField,$primaryTable,$primaryIdName,$objectType){
        if($this->get('sortable')){
            $joins["module_form_decimal form_{$this->key}"] = [
            ...
            $groupField[$this->key] = "max(form_{$this->key}.value+0) as form_{$this->key}_value";
        }
    }

    public function handleClearData($objectType,$objectId){
        db_default("module_form_decimal")->where(array(
    ...
    public function handleSaveData($objectType,$objectId,$values){
        ...
        db_default("module_form_decimal")->insert(array(
            'object_type'=>$objectType,
            'object_id'=>$objectId,
            'field_key'=>$this->key,
            'value'=>$value,
    ...
    public static function generateScript(){
        ...
        <script type="text/javascript">
            (function($,editor){
                editor.generate_decimal_field = function(fieldKey,data,cel){
                    data = $.extend({
                        'title':'未命名',
                        'description':'',
                        'readonly':false,
                    ...
                    return '<div data-field="'+fieldKey+'" data-type="decimal" class="form-editor-field fui-form-builder--col fui-form-builder--col-'+(12/cel)+' ui-draggable ui-draggable-handle" >' +
                ...
                editor.get_decimal_config=function(fieldKey,data){
                    return {
                        'base':{
                            'title':'标准',
                            'options': {
