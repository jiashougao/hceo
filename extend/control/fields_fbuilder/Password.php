...
class Password extends \control\fields\Password
{
    /**
     * 对数据查询进行排序
    ...
    public static function generateScript(){
        ...
        <script type="text/javascript">
            (function($,editor){
                editor.generate_password_field = function(fieldKey,data,cel){
                    data = $.extend({
                        'title':'未命名',
                        'description':'',
                        'options':[],
                    ...
                    let html =  '<div data-field="'+fieldKey+'" data-type="password" class="form-editor-field fui-form-builder--col fui-form-builder--col-'+(12/cel)+' ui-draggable ui-draggable-handle" >' +
                    ...
                    for(let key in data.options){
                        html+= '<label style="margin:5px;"><input type="radio" name="'+fieldKey+'" value="'+key+'">  '+data.options[key]+'</label>';
                    }
                    html+= '<span class="sui-description">'+data.description+'</span>'+
                        '</div>'+
                        '</div>';
                    return html;
                };

                editor.get_password_config=function(fieldKey,data){
                    return {
                        'base':{
                            'title':'标准',
                            'options': {
