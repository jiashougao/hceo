...
class Download extends \control\fields\Download {
    ...
    public static function generateScript() {
        ...
        <script type="text/javascript">
            (function ($, editor) {
                editor.generate_download_field = function (fieldKey, data, cel) {
                    data = $.extend({
                        'title': '未命名',
                        'description': '',
                        'btn_title': '文件下载',
                    ...
                    let html = '<div data-field="' + fieldKey + '" data-type="download" class="form-editor-field fui-form-builder--col fui-form-builder--col-' + (12 / cel) + ' ui-draggable ui-draggable-handle" >' +
                    ...
                    html += '<div style="margin:10px 10px;"><a class="btn btn-primary" href="'+data.file+'"  download>'+(data.btn_title?data.btn_title:'文件下载')+'</a></div>';
                    html += '</div>';
                    html += '</div>' +
                        '</div>';
                    return html;
                ...
                editor.get_download_config = function (fieldKey, data) {
                    return {
                        'base': {
                            'title': '标准',
                            'options': {
