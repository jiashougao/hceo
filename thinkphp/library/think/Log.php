...
class Log implements LoggerInterface
{
    ...
    function parse_class($name){
        $names = explode('\\', $name);
        return $name.'   '.end($names);
    }

    function parse_file($file, $line)
    {
        return "{$file} line {$line}".'   >  '.basename($file)." line {$line}";
    }
    /**
     * 记录日志信息
     * @access public
            ...
            if($trace){
                ...
                foreach ((array) $trace as $value) {
                    ...
                    if (isset($value['file']) && isset($value['line'])) {
                        $msg.= sprintf(' in %s', $this->parse_file($value['file'], $value['line']))."\r\n";
                    }
                }
            }


            // $msg.="\r\n 堆栈信息：".print_r($data,true);
        }
     ...
     * 记录错误信息
     * @access public
     * @param  string|Exception  $message   日志信息
     * @param  array  $context   替换内容
     * @return void
     */
